...
#   "generate" "-o" "/home/jamie/Misc/cratesioout/apollo-client/v0.5.0.nix.tmp"
  ...
  internal = rec {
    ...
    crates = {
      ...
      "apollo-client" = rec {
        ...
        features = {
          "default" = [ "host-extend" "with-curl" ];
          "full" = [ "full-curl" ];
          "full-curl" = [ "with-curl" "yaml" "xml" "host-extend" ];
          "full-hyper" = [ "with-hyper" "yaml" "xml" "host-extend" ];
          "host-extend" = [ "host-ip" "host-name" ];
          "host-ip" = [ "systemstat" "regex" ];
          "host-name" = [ "hostname" ];
          "with-curl" = [ "isahc" "futures-timer" "http-01" ];
          "with-hyper" = [ "tokio" "hyper" "http-02" ];
          "xml" = [ "serde-xml-rs" ];
          "yaml" = [ "serde_yaml" ];
        };
        resolvedDefaultFeatures = [ "default" "full" "full-curl" "full-hyper" "futures-timer" "host-extend" "host-ip" "host-name" "hostname" "http-01" "http-02" "hyper" "isahc" "mock-listen" "regex" "serde-xml-rs" "serde_yaml" "systemstat" "tokio" "with-curl" "with-hyper" "xml" "yaml" ];
      };
      "arc-swap" = rec {
        crateName = "arc-swap";
        version = "0.4.4";
